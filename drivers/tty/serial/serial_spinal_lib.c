...
//	struct spinal_lib_uart *pp = container_of(port, struct spinal_lib_uart, port);
	...
	config |= (termios->c_cflag & CSTOPB) ? SPINAL_LIB_UART_FRAME_CONFIG_STOP2 : SPINAL_LIB_UART_FRAME_CONFIG_STOP1;
	if(termios->c_cflag & CSTOPB){
		config |= (termios->c_cflag & PARODD) ? SPINAL_LIB_UART_FRAME_CONFIG_ODD : SPINAL_LIB_UART_FRAME_CONFIG_EVEN;
	}
...
//	printk("RX => %x %x", flags, pp->break_on_rx);
	if(!pp->break_on_rx){
		if(flags & SPINAL_LIB_UART_MISC_BREAK_DETECTED){
			writel(SPINAL_LIB_UART_MISC_BREAK_DETECTED, port->membase + SPINAL_LIB_UART_FRAME_FLAGS);
			pp->break_on_rx = 1;
			port->icount.brk++;
...
static int __init spinal_lib_uart_console_setup(struct console *co, char *options)
{
	...
	if (co->index < 0 || co->index >= CONFIG_SERIAL_SPINAL_LIB_UART_MAXPORTS)
		return -EINVAL;
					     ...
					     const char *options)
{
	...
	if (dev->baud) {
		...
		spinal_lib_uart_writel(port, baudclk, SPINAL_LIB_UART_DIVISOR_REG);
	}
